...
* 配置证书
  ...
  * 关于 阿里云 ssl 证书 这里选择下载 Apache 和 Nginx 的两个证书压缩包 
    * `_public.crt` 文件是证书文件。
...
```
cp 3258931_rancher.jicki.me_public.crt tls.crt
cp 3258931_rancher.jicki.me.key tls.key
cp 3258931_rancher.jicki.me.pem cacerts.pem
...
```
[root@k8s-node-1 ssl]# kubectl get secret -n cattle-system
NAME                  TYPE                                  DATA   AGE
default-token-dsctt   kubernetes.io/service-account-token   3      15h
rancher-token-fzhkn   kubernetes.io/service-account-token   3      15h
tls-ca                Opaque                                1      6s
tls-rancher           kubernetes.io/tls                     2      15h
tls-rancher-ingress   kubernetes.io/tls                     2      31s
```
