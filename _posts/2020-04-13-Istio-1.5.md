...
* 下载需要升级的 istio 版本 `curl -L https://istio.io/downloadIstio | ISTIO_VERSION=1.5.1 sh -`

* 替换 `istioctl` 二进制文件, 或者 更改 `istio` 的环境变量PATH路径到新版本的目录中。

* 如果配置了 `istioctl` 自动补全,还需要替换为 新的 自动补全脚本。

* 使用新的 `istioctl` 导出新版本的 profile 文件 `istioctl profile dump demo > demo.yaml`

* 修改 `demo.yaml` 文件, 将其中 `jwtPolicy` 身份验证机构修改为 `first-party-jwt`。Istio 将默认使用第三方令牌。

  * 验证是否支持 第三方令牌。`kubectl get --raw /api/v1 | jq '.resources[] | select(.name | index("serviceaccounts/token"))'`

    * jwtPolicy = `third-party-jwt` 使用第三方令牌 更安全 Istio 默认使用这个选项。

    * jwtPolicy = `first-party-jwt` 使用第一方令牌 属性比较不安全。

* `istioctl upgrade -f demo.yaml` 命令进行升级。
  
* 观察 `kubernets` 中 `istio-system` 的服务更新完成。

* 重新注入环境中部署的服务, 用以更新注入数据。
  ...
  * 对于手动注入的情况( 需要重新 apply 一下服务) :

    * `kubectl apply -f <(istioctl kube-inject -f nginx-test.yaml)`

* 检查升级, 执行 `istioctl version` 检查

  * `client version` 版本是否为新版本。

  * `control plane version` 版本是否为新版本。

  * `data plane version` 中是否全部 `proxies` 都为新版本。
