...
5. 修改 `demo.yaml` 文件, 将其中 `jwtPolicy` 身份验证机构修改为 `first-party-jwt`。Istio 将默认使用第三方令牌。
  ...
  1. 验证是否支持 第三方令牌。`kubectl get --raw /api/v1 | jq '.resources[] | select(.name | index("serviceaccounts/token"))'`

    1. jwtPolicy = `third-party-jwt` 使用第三方令牌 更安全 Istio 默认使用这个选项。

    2. jwtPolicy = `first-party-jwt` 使用第一方令牌 属性比较不安全。
...
8. 重新注入环境中部署的服务, 用以更新注入数据。
  ...
  1. 对于自动注入的情况可使用如下命令:
    ...
    2. `kubectl rollout restart statefulset`

    3. `kubectl rollout restart daemonset`

  2. 对于手动注入的情况( 需要重新 apply 一下服务) :

    1. `kubectl apply -f <(istioctl kube-inject -f nginx-test.yaml)`

9. 检查升级, 执行 `istioctl version` 检查

  1. `client version` 版本是否为新版本。

  2. `control plane version` 版本是否为新版本。

  3. `data plane version` 中是否全部 `proxies` 都为新版本。
